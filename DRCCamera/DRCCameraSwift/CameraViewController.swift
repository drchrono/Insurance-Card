...
//        overlay = overlay.imageByCompositingOverImage(ciImage)
    ...
    func setupCaptureSession() -> Bool{
        ...
        let videoDataOutputQueue = DispatchQueue(label: "videoOutputQueue", attributes: DispatchQueueAttributes.serial)
...
//            connection?.videoOrientation = .LandscapeRight
        ...
        DispatchQueue.global(Int(UInt64(DispatchQueueAttributes.qosUserInitiated.rawValue)), 0).async { () -> Void in
            switch UIApplication.shared().statusBarOrientation{
            case .portrait:
                connection?.videoOrientation = AVCaptureVideoOrientation.portrait
            case .portraitUpsideDown:
